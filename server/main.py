...
from ai import generate_example_call, stream_explanation
from models import ExplainRequest, RunRequest, RunResponse
...
async def run_node_code(request: RunRequest, api_key: str | None = None):
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                f"{node_runner_url}/trace", json={"code": code, "language": language}
            )
...
async def run_python_code(request: RunRequest, api_key: str | None = None):
    code = request.code
    modified_code = None
...
async def explain_code(request: ExplainRequest, http_request: Request):
    ...
    return StreamingResponse(
        stream_explanation(request.code, request.trace, api_key=api_key), media_type="text/plain"
    )
